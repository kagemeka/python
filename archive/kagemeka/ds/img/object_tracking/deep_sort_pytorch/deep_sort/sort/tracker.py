...
class Tracker:
  ...
  def update(self, detections):
    ...
    for track_idx, detection_idx in matches:
      ...
      '''
      カルマンフィルターにより、マッチング結果を与えることで環境を更新
      '''

    for track_idx in unmatched_tracks:
      self.tracks[track_idx].mark_missed()
      '''tentativeか、max_ageを超えていたらdeletedにする'''
  ...
  def _match(self, detections):
    ...
    '''これはconfirmed_track_indices'''
    confirmed_tracks = [
      i for i, t in enumerate(self.tracks) if t.is_confirmed()]

    # tentative or deleted 
    unconfirmed_tracks = [
      i for i, t in enumerate(self.tracks) if not t.is_confirmed()] # tentative || deleted
